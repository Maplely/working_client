...
public class ConverUtil {
     ...
     * @param list
     * @param ctx
     * @param flag 0旧数据库  1 新数据库
     * @return
     */
    public static HashMap<String, String> converInternetData(List<AVObject> list, Context ctx, int flag) {
        StringBuffer sb = new StringBuffer();
        mDao=null;
        if (flag == 0) {
            //旧数据
            mDao = new WorkingSheetDao_old(ctx);
        } else if (flag == 1) {
     ...
     * @param list
     * @param ctx
     * @param flag 0 旧数据库  1 新数据库
     * @return
     */
    public static HashMap<String, String> converDatabaseData(List<WorkingSheet> list, Context ctx, int flag) {
        if (flag == 0 && null == mDao) {
            mDao = new WorkingSheetDao_old(ctx);
        } else if (flag == 1 && null == mDao) {
            mDao = new WorkingSheetDao_new(ctx);
        }
        for (int i = 0; i < list.size(); i++) {
